...
def load_graph_and_complexes():
    myGraphName = "./res_myGraph"
    with open(myGraphName, 'rb') as f:
        humap = pickle_load(f)
    
    train_complex_path ="./res_train_known_complex_nodes_list"
            
    with open(train_complex_path, 'rb') as f:
        train_complex_list = pickle_load(f)
                
    test_complex_path =  "./res_test_known_complex_nodes_list"
...
def downsample_negatives(neg_train_comp_list_orig, neg_test_comp_list_orig, train_complex_list, test_complex_list):
...
def model_training(model,embed, opt, embedding_size, trainloader,valloader, val_set, prev_epochs = 0,n_epochs_to_train = 10):
    ...
    for epoch in range(n_epochs_to_train):
        ...
        logger.info("Epoch: {} Train loss: {} Validation Accuracy: {}".format(prev_epochs+epoch, cumulative_loss_train / len(trainloader),(acc*100)))
    return model
...
def evaluate(test_predictions, test_gt_lbls, correct):
    acc = correct / len(test_predictions)
    
    logger.info('Test Accuracy: %f' % (acc*100))
    logger.info("Test F1 Score : ", f1_score(test_gt_lbls, test_predictions))
    logger.info("Test APS Score : ", average_precision_score(test_gt_lbls, test_predictions))
...
def main():
    ...
    humap, train_complex_list, test_complex_list = load_graph_and_complexes()
    
    neg_train_comp_list_orig, neg_test_comp_list_orig = load_negatives()
    neg_train_comp_list, neg_test_comp_list = downsample_negatives(neg_train_comp_list_orig, neg_test_comp_list_orig, train_complex_list, test_complex_list)
    ...
    model = model_training(model,embed, opt, embedding_size, trainloader,valloader, val_set, prev_epochs = 0,n_epochs_to_train = 10)
    correct, test_predictions, test_gt_lbls = model_testing(model, testloader)
    evaluate(test_predictions, test_gt_lbls, correct)
